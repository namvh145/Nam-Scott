...
class QuangNam(scrapy.Spider):
    name = "KhanhHoa"
    start_urls = [
        "https://danangaz.com/du-lich/63-tinh-thanh/dia-diem-du-lich-khanh-hoa/"
    ]
    ...
    def parse(self, response, **kwargs):
        ...
        source = [s for s in source if not bool(re.findall(">>> Click xem ngay: ", s))]

        imgs = response.css("figure img::attr(src), p img::attr(src)").extract()
        imgs = [img for img in imgs if not  bool(re.match('gif', img[-3:]))][:-2]
